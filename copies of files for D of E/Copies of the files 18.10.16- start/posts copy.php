...
//open the "data_for_comments.json" file for writing
...
if (isset($_POST["php"])){
	...
	$Ajax_data_writing = fopen("data_for_comments.json","w");

$select_from_blog = "SELECT * FROM blog ORDER BY blog_id DESC LIMIT 25;";
...
fwrite($Ajax_data_writing, ' [" '.$row['posts'].' - sent by '.$row['user_name'].' " , "'.$row['blog_id'].'"] , ');


}; 

fwrite($Ajax_data_writing, "\n\n");
fwrite($Ajax_data_writing, ' "end of comments for posts" ] } ');
fwrite($Ajax_data_writing, "\n\n");
...
$sql_select_last = "SELECT * FROM `blog` ORDER BY blog_id DESC LIMIT 1;";
...
fwrite($Ajax_data_writing, '{"Posts" :"", "update" : ["'.$row1['posts'].' - sent by '.$row1['user_name'].'", "'.$row1['blog_id'].'"]} ');
